 ...
 * @return Noise value in the range[-1; 1], value of 0 on all integer coordinates.
 */
float SimplexNoise::fractal2D(size_t octaves, float scale, float x, float y) const {
    //float scale = 0.01f;
    float output = 0.f;
    float denom  = 0.f;
    float frequency = mFrequency;
    ...
    for (size_t i = 0; i < octaves; i++) {
        output += (amplitude * noise(x * scale * frequency, y * scale * frequency));
        denom += amplitude;
 ...
 * @return Noise value in the range[-1; 1], value of 0 on all integer coordinates.
 */
float SimplexNoise::fractal3D(size_t octaves, float scale, float x, float y, float z) const {
    float output = 0.f;
    float denom  = 0.f;
    float frequency = mFrequency;
    float amplitude = mAmplitude;
